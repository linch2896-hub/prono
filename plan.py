...
async def send_weekly_report(context: ContextTypes.DEFAULT_TYPE):
    now = datetime.now()
    if now.weekday() != 6 or now.hour != 20 or now.minute > 2:  # Воскресенье в 20:00
        return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    greeting = get_greeting_by_time()
    text = (f"{greeting} {name}! Я **Prono** — твой личный помощник.\n\n"
            f" **Уровень {users_data[chat_id]['level']}** | ✨ {users_data[chat_id]['xp']} XP\n\n"
            f"📋 **Команды:**\n"
...
def main():
    ...
    print(" Prono (полная версия) запущен!")
